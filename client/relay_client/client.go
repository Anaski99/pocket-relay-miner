...
//   - serviceID: Service identifier (e.g., "anvil", "eth-mainnet")
...
func (c *RelayClient) BuildRelayRequest(
	...
	// 2. Get current session (use cached if available)
	session, err := c.getSession(ctx, &app, serviceID)
	if err != nil {
...
func (c *RelayClient) getSession(
	...
	// Fetch session from chain
	session, err := c.queryClients.Session().GetSession(ctx, c.appAddress, serviceID, 0)
	if err != nil {
