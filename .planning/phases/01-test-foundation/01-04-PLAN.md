...
3. File contains sections for: time.Sleep, Global State, Non-Deterministic Data, Coverage Gaps, Flaky Tests
...
4. Update the audit document's "Flaky Tests Identified" section with results:
   - If no flaky tests found: Document "No flaky tests detected in 100-run stability check"
   - If flaky tests found: List each one with package, failure rate, and skip status

5. Commit message for any test skips should reference this plan:
...
Per 01-CONTEXT.md: "Policy for existing flaky tests: Skip with t.Skip() and TODO comment, fix in Phase 3"
  ...
  <verify>
1. `RUNS=10 ./scripts/test-stability.sh` completes (quick validation)
2. Full 100-run stability test has been run (check audit document for results)
3. Any flaky tests are either:
   - Fixed (if trivial), OR
   - Skipped with `t.Skip("TODO(phase3): ...")` and documented in audit
4. Audit document's "Flaky Tests Identified" section is filled in (even if "none found")
  </verify>
  <done>
Stability test completed. All flaky tests either fixed or skipped with TODO comments. Audit document updated with results.
  </done>
</task>
...
4. Stability test was run (100 runs) and results documented
