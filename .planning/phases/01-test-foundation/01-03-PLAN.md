...
<objective>
Update CI pipeline with race detection, vulnerability scanning, and nightly stability checks

Purpose: Complete the CI infrastructure for Phase 1 by adding govulncheck (INFRA-03), ensuring race detection runs on every PR (INFRA-01), and creating a nightly stability job for flaky test detection (QUAL-02). These changes make the quality gates enforceable.
...
<task type="auto">
  <name>Task 1: Update CI workflow with govulncheck and race tests</name>
...
2. Update the `test` job to explicitly use race detection:
   - Change the test command from `make test` to explicitly call `go test -race -tags test ./...`
   - This ensures race detection even if Makefile changes
   - Keep coverage generation separate (after race-enabled tests pass)
...
3. test job includes `-race` flag
4. docker-build jobs depend on vuln-check
  </verify>
  <done>
CI workflow updated with govulncheck job and race-enabled tests. Docker builds now require vuln-check to pass.
  </done>
</task>
...
2. CI test job explicitly uses `-race` flag
...
- CI test job runs with race detection enabled
